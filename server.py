...
def has_oauth_credentials() -> bool:
    """Hermes OAuth providers (Codex, Gemini CLI, Claude Code, Copilot, ...)
    store credentials in $HERMES_HOME/auth.json instead of env-var API keys,
    so the PROVIDER_KEYS check below misses them. Treat any populated
    credential_pool / providers entry as a valid provider.
    """
    auth_file = Path(os.environ.get("HERMES_HOME", "/data/.hermes")) / "auth.json"
    try:
        data = json.loads(auth_file.read_text())
    except (FileNotFoundError, json.JSONDecodeError):
        return False
    return bool(data.get("credential_pool")) or bool(data.get("providers"))
